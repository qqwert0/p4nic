...
class NetworkStackSpa extends Module {
	...
	val LrouterL1			= SimpleRouter(new Idx(), CONFIG.ENG_NUM/16)
	val LrouterL2			= Seq.fill(CONFIG.ENG_NUM/16)(SimpleRouter(new Idx(),4))
    val LrouterL3			= Seq.fill(CONFIG.ENG_NUM/4)(SimpleRouter(new Idx(),4))
	val GrouterL1			= SimpleRouter(new Idx(), CONFIG.ENG_NUM/16)
	val GrouterL2			= Seq.fill(CONFIG.ENG_NUM/16)(SimpleRouter(new Idx(),4))
    val GrouterL3			= Seq.fill(CONFIG.ENG_NUM/4)(SimpleRouter(new Idx(),4))
    val arbiterL1			= XArbiter(new Meta(),CONFIG.ENG_NUM/16)
    val arbiterL2			= XArbiter(CONFIG.ENG_NUM/16)(new Meta(),4)
    val arbiterL3			= XArbiter(CONFIG.ENG_NUM/4)(new Meta(),4)
    ...
    data_split.io.IndexOut.bits.engine_idx(6,4)      <>  LrouterL1.io.idx
	...
	for(i<-0 until CONFIG.ENG_NUM/16){
		LrouterL1.io.out(i)	<> LrouterL2(i).io.in 
		LrouterL2(i).io.idx	<> LrouterL2(i).io.in.bits.engine_idx(3,2)
		for(j<-0 until 4){
			LrouterL2(i).io.out(j)	        <> LrouterL3(i*4+j).io.in
            LrouterL3(i*4+j).io.idx	        <> LrouterL3(i*4+j).io.in.bits.engine_idx(1,0)
            for(m<-0 until 4){
                LrouterL3(i*4+j).io.out(m).bits.block_idx	<> gen_header(i*16+j*4+m).io.LocalIndex.bits
                LrouterL3(i*4+j).io.out(m).ready	<> gen_header(i*16+j*4+m).io.LocalIndex.ready
                LrouterL3(i*4+j).io.out(m).valid	<> gen_header(i*16+j*4+m).io.LocalIndex.valid
                gen_header(i*16+j*4+m).io.Bitmap        <>  io.nodeRank
                gen_header(i*16+j*4+m).io.EngineRank    <>  (i*16+j*4+m).U
            }
		}
	}
	...
	for(i<-0 until CONFIG.ENG_NUM/16){
		...
		GrouterL2(i).io.idx	<> GrouterL2(i).io.in.bits.engine_idx(3,2)
		for(j<-0 until 4){
			GrouterL2(i).io.out(j)	        <> GrouterL3(i*4+j).io.in
            GrouterL3(i*4+j).io.idx	        <> GrouterL3(i*4+j).io.in.bits.engine_idx(1,0)            
            for(m<-0 until 4){
                GrouterL3(i*4+j).io.out(m).bits.block_idx	<> gen_header(i*16+j*4+m).io.GlobeIndex.bits
                GrouterL3(i*4+j).io.out(m).ready	<> gen_header(i*16+j*4+m).io.GlobeIndex.ready
                GrouterL3(i*4+j).io.out(m).valid	<> gen_header(i*16+j*4+m).io.GlobeIndex.valid
            }
	...
	for(i<-0 until CONFIG.ENG_NUM/16){
		...
		for(j<-0 until 4){
            ...
            for(m<-0 until 4){
                arbiterL3(i*4+j).io.in(m)	<> RegSlice(gen_header(i*16+j*4+m).io.MetaOut)      
            }      
		}
	}
    ...
    rx_process.io.GlobalIndex.bits.engine_idx(6,4)      <>  GrouterL1.io.idx
